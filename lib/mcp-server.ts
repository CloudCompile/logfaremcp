...
import { AGENT_ROLES, listAgentRoles, resolveAgent, type AgentRole } from './agents';
import { chooseModel } from './routing';
import { chatFields, embeddingsSchema, imageSchema, listAgentsSchema, modelInfoSchema, routeSchema, spawnAgentSchema, sttSchema, ttsSchema } from './validation';
...
const isAgentRole = (value: string): value is AgentRole => value in AGENT_ROLES;
...
export function createMcpServer() {
  ...
  server.registerTool('logfare_list_agents', { description: 'Use when the user asks what specialist Logfare agents are available. Returns stable role definitions; model assignments are resolved dynamically from the live Logfare catalog when an agent is spawned.', inputSchema: fields(listAgentsSchema) }, async () => { try { return json({ agents: listAgentRoles() }); } catch (e) { return error(e); } });
  server.registerTool('logfare_spawn_agent', { description: 'Use to delegate a bounded task to a Logfare specialist such as planner, coder, reviewer, researcher, image, voice, transcriber, or embeddings. It fetches the live catalog, selects only a compatible advertised model, applies the role system prompt, and returns the result plus routing decision. It does not create persistent agents or expose arbitrary HTTP access.', inputSchema: fields(spawnAgentSchema) }, async (input) => { try { const models = await client().listModels(); if (!isAgentRole(input.role)) throw new Error('Unknown agent role'); const decision = resolveAgent(models, input.role, input.model); const role = AGENT_ROLES[input.role]; if (role.modality !== 'chat') throw new Error(`The '${input.role}' role requires its modality-specific MCP tool; agent delegation currently executes text tasks only`); const response = await client().chat({ model: decision.model.id, system_prompt: `You are the ${role.label} specialist. ${role.description} Stay within the assigned task and clearly state uncertainty.`, prompt: input.context ? `Context:\n${input.context}\n\nTask:\n${input.task}` : input.task, temperature: input.temperature, max_tokens: input.max_tokens }); return json({ role: input.role, selected_model: decision.model.id, reason: decision.reason, result: extractText(response), response }); } catch (e) { return error(e); } });
